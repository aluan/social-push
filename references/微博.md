 ...
 * @Date: 2026-03-08 22:12:03
 * @LastEditors: qiangqiang.luan qiangqiang.luan@17zuoye.com
 * @LastEditTime: 2026-03-10 22:38:54
 * @FilePath: /social-push-skill/references/微博.md
 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
-->
## 快速使用（脚本）

```bash
bash ./scripts/weibo.sh <内容> [图片路径] [话题]
# 示例
bash ./scripts/weibo.sh '今天天气真好' /path/to/image.jpg '生活记录'
```

## 发布微博 workflow

1. 打开微博主页：`agent-browser --profile /tmp/agent-profile open "https://weibo.com"`
...
8. 最后一步根据用户要求选择：
   - 发布（需用户明确确认）：`agent-browser click @e{发送按钮ref}`
   - 不发布：停留在页面，保留已编辑内容
